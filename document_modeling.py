...
def perplexity_of_corpus(K, corpus, alpha, beta):
    ...
    for doc_num in range(M):
        log_likelihood_corpus += np.log(likelihood_of_document(K, corpus[doc_num], alpha, beta))
...
def likelihood_of_document(K, document, alpha, beta):
    ''' Input:
    K = number of topics
