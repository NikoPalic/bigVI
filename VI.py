...
def VI(k, N, alpha, beta):
    ...
    while (True):
        for n in range(N):
            for i in range(k):
                phi_t1[n,i] = beta[i,n] * np.exp(digamma(gamma_0[i]))   #Unsure if correct dimensions
            phi_t1 = normalize(phi_t1, axis = 1, norm = 'l1')
        gamma_t1 = alpha + np.ndarray.sum(phi_t1,axis = 1)              #Does not work properly

        #print(np.abs(gamma_0 -gamma_t1))#<0.01:                        #Need to implement proper convergence criterion
